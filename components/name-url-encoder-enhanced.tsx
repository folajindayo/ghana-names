...
import { Link, Sparkles, RotateCcw, Copy, Check } from 'lucide-react'
import { useToast } from '@/hooks/use-toast'

export function NameUrlEncoderEnhanced() {
  ...
  const encode = () => {
    ...
    try {
      const result = encodeURIComponent(name.trim())
      setEncoded(result)
      setDecoded('')
      setError('')
      toast({
        title: "Encoded!",
        description: "Name encoded for URL",
      })
    } catch (err) {
  ...
  const decode = () => {
    if (!encoded.trim()) {
      toast({
        ...
        description: "Please enter URL encoded text",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      const result = decodeURIComponent(encoded.trim())
      setDecoded(result)
      setError('')
      toast({
        title: "Decoded!",
        description: "URL text decoded successfully",
      })
    } catch (err) {
      setError('Invalid URL encoded string')
      toast({
        title: "Error",
        description: "Invalid URL encoded text",
        variant: "destructive",
      })
    }
  }
    ...
    <Card className="bg-white/10 backdrop-blur-sm border-white/20">
      <CardHeader>
        <CardTitle className="text-white flex items-center gap-2">
          ...
          Enhanced URL Encoder
        </CardTitle>
        <CardDescription className="text-white/70">
          Encode and decode names for safe use in URLs
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {encoded && (
          <div className="p-5 bg-white/5 rounded-lg border border-white/10 space-y-4">
            ...
            <div className="p-3 bg-blue-500/10 rounded border border-blue-400/30">
              <p className="text-blue-200 text-xs">
                <strong>Example URL:</strong> https://example.com/name/{encoded}
              </p>
            </div>

            <div className="space-y-2">
              <Label className="text-white/80">Decode URL Encoded Text</Label>
        ...
        {name && !encoded && (
          <div className="p-4 bg-white/5 rounded-lg border border-white/10 text-center">
            <p className="text-white/60 text-sm">Enter a name and click to encode for URL use</p>
          </div>
        )}
      </CardContent>
    </Card>
