...
import { Lock, Sparkles, Copy, Check, Unlock } from 'lucide-react'
import { useToast } from '@/hooks/use-toast'

export function NameTextEncoderEnhanced() {
    ...
    try {
      if (encodeType === 'base64') {
        if (isEncoding) {
          ...
          encoded = decodeURIComponent(escape(atob(input)))
        }
      } else if (encodeType === 'url') {
        if (isEncoding) {
          encoded = encodeURIComponent(input)
        } else {
          encoded = decodeURIComponent(input)
        }
        ...
        if (isEncoding) {
          encoded = input.split('').map(char => `\\u${char.charCodeAt(0).toString(16).padStart(4, '0')}`).join('')
        } else {
          encoded = input.replace(/\\u([0-9a-fA-F]{4})/g, (_, hex) => String.fromCharCode(parseInt(hex, 16)))
        }
      }
    ...
    <Card className="bg-white/10 backdrop-blur-sm border-white/20">
      <CardHeader>
        <CardTitle className="text-white flex items-center gap-2">
          ...
          Enhanced Text Encoder/Decoder
        </CardTitle>
        <CardDescription className="text-white/70">
          Encode or decode text (Base64, URL, HTML, Unicode)
        </CardDescription>
      </CardHeader>
